...
mult_imp_approach = function(outcome, covar = NULL, zeros = NULL, data, family, components = "binary", m = 100, post_imputation = "none", use_glm = TRUE) {
  ...
  # Setup storage for coefficients from each post-imputation model
  ...
  if (use_glm) {
    ...
    per_imp_imp = matrix(nrow = m, ncol = p - 1) ## no intercept for random forest
  }
  ...
  for (b in 1:m) {
    ...
    if (use_glm) {
      if (use_zeroinfl) {
        term_names = names(coef(post_imp_dat_b$fit, model = "count"))
        per_imp_coeff[b, ] = as.vector(coef(post_imp_dat_b$fit, model = "count"))
        per_imp_vars[b, ]  = as.vector(diag(vcov(post_imp_dat_b$fit, model = "count")))
      } else {
        term_names = names(post_imp_dat_b$fit$coefficients)
  ...
  if (use_glm) {
    ...
    summ_fit_imp = data.frame(term = term_names,
